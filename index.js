...
async function readFacturae(file) {
  ...
  const totalInvoiceAmount = xmlDoc.getElementsByTagName("TotalAmount")[0].textContent;
  ...
  const totalWithoutTax = xmlDoc.getElementsByTagName("TotalGrossAmountBeforeTaxes")[0].textContent;
  const taxes = xmlDoc.getElementsByTagName("TotalTaxOutputs")[0].textContent;

  // We get the seller, buyer, and the lines
  ...
  // Seller data
  const sellerName = seller.getElementsByTagName("CorporateName")[0].textContent;
  ...
  for (let i = 0; i < invoiceLines.length; i++) {
    const description = invoiceLines[i].getElementsByTagName("ItemDescription")[0].textContent;
    const quantity = invoiceLines[i].getElementsByTagName("Quantity")[0].textContent;
    const price = invoiceLines[i].getElementsByTagName("UnitPriceWithoutTax")[0].textContent;
