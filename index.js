class FacturaeReader {
  ...
  async readFacturae(file) {
    ...
    const sellerProvince =
      seller.getElementsByTagName("Province")[0].textContent;
    const sellerPostCode =
      seller.getElementsByTagName("PostCode")[0].textContent;
    // Buyer data
    const buyerName =
      buyer.getElementsByTagName("CorporateName")[0].textContent;
    ...
    for (let i = 0; i < invoiceLines.length; i++) {
      ...
      const price = invoiceLines[i].getElementsByTagName(
        "UnitPriceWithoutTax"
      )[0].textContent;
      itemsFacturae.push({
        product: description,
